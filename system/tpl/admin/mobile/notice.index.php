                ...
                <div class="layui-card-header">
                    ...
                    <div class="layui-input-inline" style="width:80px;float: right;">
                        <button class="layui-btn layui-btn-sm layui-btn-normal" id="add_banner">
                        	<a href="?m=config&c=notice_add" style="text-decoration:none; color:#fff;">添加公告</a>
                        </button>
                    </div>
                </div>
...
<script>
    layui.use(['element','layer','table'], function(){
        ...
        table.render({
            elem: '#list_wen'
            ,url:'?m=config&c=notice_list'
            ,cellMinheight: 80
            ,height: 'full-110'
            ,limit: 20
            ,cols: [[
                 {field:'id', title:'ID', width:80,align:'center', unresize: true}
                ,{field:'n_title', title:'公告标题',align:'center'}
        ...
        table.on('tool(list_wen)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('删除后将不能恢复，确定删除？', function(index){
                    ...
                    layui.$.post('?m=config&c=notice_del',{id:banner_id},function(res){
                        if(res.role == 'error'){
                            alert('您当前没有操作权限');
            ...
            if(obj.event === 'edit'){
                var banner_id = data.id;
                window.location.href='/admin.php?m=config&c=notice_edit&id=' + banner_id;
            }
