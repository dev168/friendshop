        ...
        <div class="layui-card">
          <div class="layui-card-header">
              <?php if($c_type == 1){ ?>
                  ...
                  <button class="layui-btn layui-btn-sm layui-btn-normal" id="add_account">添加行业</button>
                  </span>
                  行业管理
              <?php }else{ ?>
                  <span style="float: right;">
	...
	layui.use(['element','layer','form','table'], function(){
	  ...
	  layui.$("#add_account").click(function(){
		  layer.open({
			  ...
			  area:  ['420px', '600px'],
			  shadeClose: 0,
			  content:'?m=shop&c=addcate&c_type=<?php echo $c_type;?>'
		  });
	  });
	  table.render({
		elem: '#weblist'
		,url:'?m=shop&c=catelist&c_type=<?php echo $c_type;?>'
		,cellMinheight: 80
		,height: 'full-110'
		,limit: 20
		,cols: [[
	  ...
	  table.on('tool(weblist)', function(obj){
		...
		if(obj.event === 'del'){
		  layer.confirm('删除后将不能恢复，确定删除？', function(index){
			var webid=data.id;
			layui.$.post('?m=shop&c=delcate',{'id':webid},function(retmsg){
		...
		if(obj.event === 'edit'){
		  ...
		  layer.open({
			  ...
			  area:  ['420px', '600px'],
			  shadeClose: 0,
			  content:'?m=shop&c=editcate&id='+webid
		  });
		}
	  });
	});
