...
(function (factory) {
    ...
    $.fn.orgchart = function (options) {
        ...
        if ($.type(data) === 'object') {
            ...
            $.ajax({
                'url': data, 'dataType': 'json', 'beforeSend': function () {
                ...
                'click': function () {
                    ...
                    html2canvas(sourceChart, {
                        ...
                        'onclone': function (cloneDoc) {
                            $(cloneDoc).find('.canvasContainer').css('overflow', 'visible').find('.orgchart:visible:first').css('transform', '');
                        },
                        'onrendered': function (canvas) {
                            $chartContainer.find('.mask').addClass('hidden').end().find('.oc-download-btn').attr('href', canvas.toDataURL())[0].click();
                        }
                    }).then(function () {
    ...
    function showDescendants($node) {
        var $descendants = $node.closest('tr').siblings().removeClass('hidden').eq(2).children().find('tr:first').find('.node:visible');
    ...
    function hideSiblings($node, direction) {
        ...
        $animatedNodes.eq(0).one('transitionend', function () {
            ...
            $nodeContainer.closest('.nodes').prev().children(':not(.hidden)').slice(1, direction ? $siblings.length * 2 + 1 : -1).addClass('hidden');
            $animatedNodes.removeClass('slide');
            $siblings.find('.node:visible:gt(0)').removeClass('slide-left slide-right').addClass('slide-up').end().find('.lines, .nodes').addClass('hidden').end().addClass('hidden');
            if (isInAction($node)) {
    ...
    function createNode(nodeData, level, opts) {
        var dtd = $.Deferred();
        var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + '>').addClass('node ' + (nodeData.className || '') + (level >= opts.depth ? ' slide-up' : '')).append('<div class="title">' + nodeData[opts.nodeTitle] + '</div>').append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (nodeData[opts.nodeContent] || '') + '</div>' : '');
        ...
        if (Number(flags.substr(2, 1))) {
            $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>').children('.title').prepend('<i class="fa ' + opts.parentNodeSymbol + ' symbol"></i>');
        }
        ...
        $nodeDiv.on('click', '.topEdge', function (event) {
            ...
            if (parentState.exist) {
                ...
                if (startLoading($that, $node, opts)) {
                    $.ajax({'url': opts.ajaxURL.parent + nodeId + '/', 'dataType': 'json'}).done(function (data) {
        ...
        $nodeDiv.on('click', '.leftEdge, .rightEdge', function (event) {
            ...
            if (siblingsState.exist) {
                ...
                if (startLoading($that, $node, opts)) {
                    $.ajax({'url': url + nodeId + '/', 'dataType': 'json'}).done(function (data, textStatus, jqXHR) {
                        if ($node.closest('.orgchart').data('inAjax')) {
        ...
        if (opts.draggable) {
            $nodeDiv.on('dragstart', function (event) {
                ...
                if (!$dropZone.closest('tr').siblings().length) {
                    $dropZone.append('<i class="edge verticalEdge bottomEdge fa"></i>').parent().attr('colspan', 2).parent().after('<tr class="lines"><td colspan="2"><div class="down"></div></td></tr>'
                        + '<tr class="lines"><td class="right">&nbsp;</td><td class="left">&nbsp;</td></tr>'
                        + '<tr class="nodes"></tr>').siblings(':last').append($dragged.find('.horizontalEdge').remove().end().closest('table').parent());
                } else {
                    ...
                    $dropZone.closest('tr').siblings().eq(1).children(':last').before('<td class="left top">&nbsp;</td><td class="right top">&nbsp;</td>').end().next().append($dragged.closest('table').parent());
    ...
    function buildParentNode($currentRoot, nodeData, opts, callback) {
        ...
        $.when(createNode(nodeData, 0, opts || $currentRoot.closest('.orgchart').data('options'))).done(function ($nodeDiv) {
            ...
            oc.prepend($table).children('table:first').append('<tr class="nodes"><td colspan="2"></td></tr>').children().children('tr:last').children().append(oc.children('table').last());
            callback();
        }).fail(function () {
    ...
    function complementLine($oneSibling, siblingCount, existingSibligCount) {
        ...
        $oneSibling.parent().prevAll('tr:gt(0)').children().attr('colspan', siblingCount * 2).end().next().children(':first').after(lines);
    }
        ...
        if ($nodeChart.parent().is('td')) {
            ...
            buildChildNode.call($nodeChart.closest('.orgchart').parent(), $nodeChart.parent().closest('table'), nodeData, opts, function () {
                if (++childCount === newSiblingCount) {
                    ...
                    if (existingSibligCount > 1) {
                        ...
                        $siblingTds.not(':eq(' + insertPostion + 1 + ')').addClass('hidden').slice(0, insertPostion).find('.node').addClass('slide-right').end().end().slice(insertPostion).find('.node').addClass('slide-left');
                    }
                    callback();
                }
            });
            ...
            buildHierarchy($nodeChart.closest('.orgchart'), nodeData, 0, opts, function () {
                if (++nodeCount === siblingCount) {
                    complementLine($nodeChart.next().children().children('tr:last').children().eq(insertPostion).after($('<td colspan="2">').append($nodeChart)), siblingCount, 1);
                    $nodeChart.closest('tr').siblings().eq(0).addClass('hidden').find('.node').addClass('slide-down');
                    $nodeChart.parent().siblings().addClass('hidden').slice(0, insertPostion).find('.node').addClass('slide-right').end().end().slice(insertPostion).find('.node').addClass('slide-left');
                    callback();
                }
            });
        }
    ...
    function removeNodes($node) {
        ...
        if ($parent.is('td')) {
            if (getNodeState($node, 'siblings').exist) {
                ...
                $sibs.eq(0).children().removeAttr('colspan').find('.bottomEdge').remove().end().end().siblings().remove();
            }
        } else {
